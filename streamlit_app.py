...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------- Ingest
        ...
        try:
            ...
            if not keys:
                raise RuntimeError("list_sources() returned no sources")
            doc_options = sorted(
            ...
            docs_df = run_opt(
                ...
                "ORDER BY institution, doc_id")
            doc_options = sorted(
                ((f"{_bank_of(r.institution)} · {r.doc_id}", r.source_file)
                 for r in docs_df.itertuples()),
                key=lambda p: p[0],
            )
            src_mode = "document table (local fallback — list_sources() unavailable/empty)"

        st.caption(f"Document source: {src_mode}")

        if not doc_options:
            st.info("No documents available from either source.")
            st.markdown("</div>", unsafe_allow_html=True)
        else:
            labels = [lbl for lbl, _ in doc_options]
            ...
            if st.button("Ingest document"):
                ...
                proc = subprocess.Popen(
                    [sys.executable, "findociq/pipeline/run_doc.py",
                     "--pdf", selected_key, "--no-ipv4-shim"],
                    cwd=str(REPO),
                )
                st.session_state["ingest_proc"] = proc
                st.session_state["ingest_source_file"] = selected_key
                st.session_state["ingest_doc_id"] = doc_id
            st.markdown("</div>", unsafe_allow_html=True)

            # ---------------------------------------------- live stepper
            active_source_file = st.session_state.get("ingest_source_file", selected_key)
            st.markdown(
                '<div class="card"><div class="card-title">3. Progress</div>',
                unsafe_allow_html=True,
            )
            # source_file in ingest_status is written by run_doc.py verbatim
            # (via source_store.key_for()); the corpus has been observed with
            # both a bare "financial_statements/X.pdf" key AND a
            # "findociq/data/sources/financial_statements/X.pdf" key, so match
            # on the stable trailing "<folder>/<file>.pdf" suffix rather than
            # assuming one exact prefix convention.
            suffix = "/".join(Path(active_source_file).parts[-2:])
            status_rows_df = run_opt(
                f"SELECT stage, state, error_message, attempt_count "
                f"FROM {TBL('ingest_status')} "
                f"WHERE source_file = '{_esc(active_source_file)}' "
                f"   OR source_file LIKE '%{_esc(suffix)}'")
            status_rows = status_rows_df.to_dict("records") if not status_rows_df.empty else []
            states = stage_states(status_rows, STAGES)
            for stage, state, message in states:
                if state == "done":
                    st.markdown(f'<div class="stage-done">✓ {stage}</div>',
                                unsafe_allow_html=True)
                elif state == "running":
                    with st.spinner(f"{stage}"):
                        st.markdown(f'<div>{stage}</div>', unsafe_allow_html=True)
                elif state == "failed":
                    st.markdown(
                        f'<div class="stage-failed">✕ {stage} — {message}</div>',
                        unsafe_allow_html=True)
                else:
                    st.markdown(f'<div class="stage-pending">• {stage}</div>',
                                unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)

            proc = st.session_state.get("ingest_proc")
            run_active = proc is not None and proc.poll() is None
            terminal = states[-1][1] == "done" or any(s == "failed" for _, s, _ in states)
            if run_active and not terminal:
                time.sleep(2)
                st.rerun()

            # ---------------------------------------------------- on done
            doc_is_ingested = any(s == "done" for _, s, _ in states)
            if doc_is_ingested:
                st.markdown(
                    '<div class="card"><div class="card-title">4. Result</div>',
                    unsafe_allow_html=True,
                )
                tbls = run_opt(
                    f"SELECT table_id, table_title, table_type, page_range "
                    f"FROM {TBL('table_t')} WHERE doc_id = '{_esc(doc_id)}' "
                    f"ORDER BY table_id")
                if tbls.empty:
                    ...
                    if conn is not None:
                        family_row = run_opt(
                            ...
                            f"WHERE doc_id = '{_esc(doc_id)}'")
                        family = (family_row["doc_family"].iloc[0]
                                  if not family_row.empty else None)
                        col_a, col_b = st.columns(2)
                        with col_a:
                            xlsx_path = None
                            try:
                                if family == "income_statement":
                                    from tag_workbook import build_workbook
                                    wb, _meta = build_workbook(DB, family=family)
                                    xlsx_path = Path(tempfile.mkstemp(suffix=".xlsx")[1])
                                    wb.save(xlsx_path)
                                else:
                                    raise ValueError("family not supported by build_workbook")
                            except Exception:
                                xlsx_path = _plain_xlsx_export(conn, doc_id)
                            st.download_button(
                                "Download Excel (for finance mapping)",
                                data=xlsx_path.read_bytes(),
                                file_name=f"{doc_id}.xlsx",
                                mime="application/vnd.openxmlformats-officedocument"
                                     ".spreadsheetml.sheet",
                            )
                        with col_b:
                            csv_text = doc_to_csv(conn, doc_id)
                            st.download_button(
                                "Download CSV",
                                data=csv_text,
